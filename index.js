...
app.post('/place_order', function (req, res) {
    ...
    con.connect((err) => {
        if (err) {
            ...
            var query = "INSERT INTO orders(id,name,email,status,city,address,cost,phone,date,products_ids) VALUES ?";
            var values = [
                [id, name, email, status, city, address, cost, phone, date, products_ids]
            ];

            con.query(query, [values], (err, result) => {
                for (let i = 0; i < cart.length; i++) {
                    var query = "INSERT INTO order_items (order_id,product_id,product_name,product_price,product_image,product_quantity,order_date)";
                    var values = [
